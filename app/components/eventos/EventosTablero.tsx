...
export function EventosTablero({
  ...
  return (
    <div className="space-y-5">
      {!summary?.ready && summary?.error && (
        <SuiteCard accent>
          <p className="text-sm font-semibold" style={{ color: theme.title }}>
            Base de datos pendiente
          </p>
          <p className="mt-1 text-sm" style={{ color: theme.muted }}>
            Ejecuta <code className="text-xs">supabase/eventos_module.sql</code> en
            el SQL Editor de Supabase (crea tablas + seed de menús). Luego recarga
            y, en CRM, usa «Importar Excel seed» si no hay clientes. Detalle:{' '}
            {summary.error}
          </p>
        </SuiteCard>
      )}
              ...
              {summary!.upcomingEvents.map((ev) => (
                ...
                >
                  ...
                  <div className="mt-0.5 text-xs text-slate-500">
                    ...
                    {ev.estimated_amount
                      ? ` · ${formatMxn(Number(ev.estimated_amount))}`
                      : ''}
                  </div>
                </li>
              ))}
