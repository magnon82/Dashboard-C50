...
| `menu-c50-esp.pdf` / `c50-esp.txt` | `Menú C50\Menú C50 Esp.pdf` | Menú regular alimentos (`menu_regular_c50`) + bebidas |
| `alimentos-c50-items.json` | Generado | Ítems del menú regular |
| `bebidas-c50-items.json` | Generado | Ítems del catálogo bebidas |

Regenerar C50 (alimentos + bebidas):

```bash
python scripts/build_eventos_alimentos_c50.py
...
En Supabase: ejecutar `eventos_module.sql` (bloque menús), `eventos_menus_regular_c50.sql` (category carta + menú) y luego `eventos_menus_bebidas_c50.sql`.
