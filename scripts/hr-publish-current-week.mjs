/**
 * Pasa semanas pasadas + en curso (lunes ISO CDMX) de borrador/propuesta → publicado.
 *
 * Uso:
 *   node scripts/hr-publish-current-week.mjs
 *   node scripts/hr-publish-current-week.mjs --dry-run
 *   node scripts/hr-publish-current-week.mjs --year=2026
 *
 * Requiere .env.local (NEXT_PUBLIC_SUPABASE_URL + service role).
 */
...
const { data: updated, error: uErr } = await sb
  ...
  .update({
    status: 'publicado',
    published_by: 'script-publish-current',
    published_at: nowIso,
    updated_at: nowIso,
  })
  .in('id', ids)
...
console.log(`Actualizadas: ${updated?.length ?? 0}`);
